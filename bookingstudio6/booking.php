...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (mysqli_num_rows($cek) > 0) {
        ...
        if (mail($to, $subject, $emailMessage, $headers)) {
            // Save booking
            $insert = mysqli_query($conn, "INSERT INTO bookings (id, id_studio, tanggal, jam_mulai) VALUES ('$id', '$id_studio', '$tanggal', '$jam')");
            $message = "<p class='success'>✅ Booking berhasil! Konfirmasi dikirim ke email Anda.</p>";
        } else {
            $message = "<p class='error'>⚠️ Booking Gagal, Email Konfirmasi tidak dapat dikirim.</p>";
        }
    }
}
?>
